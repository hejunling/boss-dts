...
public class ProviceUnicomVoiceServiceImpl implements IProviceUnicomVoiceService {

    private static final Logger log = LoggerFactory.getLogger(ProviceUnicomVoiceServiceImpl.class);
    ...
    private BizUserVoiceInfo uniVoiceToBossVoice(UserVoiceInfo userVoiceInfo){
        ...
        String bizno = CacheConstant.LT_PROVINCE_MAP.get(userVoiceInfo.getRpcode());
    ...
    private BizUserVoiceInfo uniEntVoiceToBossVoice(EntUserVoiceInfo entUserVoiceInfo){
        ...
        String bizno = CacheConstant.LT_PROVINCE_MAP.get(entUserVoiceInfo.getRpcode());
        if(StringUtil.isNotBlank(bizno)){
            bizUserVoiceInfo.setBizNo( bizno); // 业务编号
        }else {
            throw new CustomException("PROVICE_BIZ 中rpcode对应的Bizno不存在");
    ...
    private void insertUserVoiceQueue(List<UserVoiceInfo> userVoiceInfoList){
        ...
        for (UserVoiceInfo userVoiceInfo: userVoiceInfoList) {
            ...
            try {
                ...
                log.debug("联通分省个人录音字段对应boss字段异常 = {},userVoiceInfo = {}",e,userVoiceInfo);
                errorInfo = "联通分省个人录音字段对应boss字段异常:"+ e.getMessage() + " userVoiceInfo:"+userVoiceInfo.toString();
            }
            bossVoiceInfo.setBizname("联通分省个人");
    ...
    private void insertEntUserVoiceQueue(List<EntUserVoiceInfo> entUserVoiceInfos){
        ...
        for (EntUserVoiceInfo entUserVoiceInfo: entUserVoiceInfos) {
            ...
            try {
                ...
                log.debug("联通分省企业录音字段对应boss字段异常 = {},entUserVoiceInfo = {}",e,entUserVoiceInfo);
                errorInfo = "联通分省企业录音字段对应boss字段异常:"+ e.getMessage() + " entUserVoiceInfo:"+entUserVoiceInfo.toString();
            }
            bossVoiceInfo.setBizname("联通分省企业");
            bossVoiceInfo.setUuid(uuid);
            bossVoiceInfo.setBizUserVoiceInfo(bizUserVoiceInfo);
