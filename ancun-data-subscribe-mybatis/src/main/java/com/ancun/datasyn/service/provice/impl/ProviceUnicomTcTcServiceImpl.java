...
public class ProviceUnicomTcTcServiceImpl implements IProviceUnicomTcService {

    private static final Logger log = LoggerFactory.getLogger(ProviceUnicomTcTcServiceImpl.class);
    ...
    public void insertProviceTcInfoQ(TaocanInput input){
        // 得到需要同步的列表
        List<UserTaocan> userTaocen = getSynTaocanList(input);
        checkList(userTaocen);
        log.info("同步分省联通套餐列表 "+ userTaocen.size() + " 条");
        // 将信息插入到队列中
        insertQueue(userTaocen);
    ...
    private BizComboInfo uniTcToBossTc(UserTaocan userTaocan){
        ...
        String bizno = CacheConstant.LT_PROVINCE_MAP.get(userTaocan.getRpcode());
    ...
    private void insertQueue(List<UserTaocan> userTcList){
        ...
        for (UserTaocan userTaocan: userTcList) {
            ...
            try {
                ...
                log.debug("联通分省字段对应boss字段异常 = {},userTaocan = {}",e,userTaocan);
                errorInfo = "联通分省字段对应boss字段异常:"+ e.getMessage() + " userTaocan:" + userTaocan;
            }
            bossTaocanInfo.setBizname("联通分省");
