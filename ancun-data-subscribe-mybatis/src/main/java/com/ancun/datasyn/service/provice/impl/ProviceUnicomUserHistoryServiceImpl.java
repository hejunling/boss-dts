...
public class ProviceUnicomUserHistoryServiceImpl implements IProviceUnicomUserHistoryService {

    private static final Logger log = LoggerFactory.getLogger(ProviceUnicomUserHistoryServiceImpl.class);
     ...
     * @return
     */
    private BizUserLifeCircle unicomUserHistoryToBoss(UserInfoHistory userInfoHistory){
        ...
        String bizno = CacheConstant.LT_PROVINCE_MAP.get(userInfoHistory.getRpcode());
     ...
     * @return
     */
    private BizUserLifeCircle unicomEntUserHistoryToBoss(EntUserInfoHistory entUserInfoHistory){
        BizUserLifeCircle bizUserLifeCircle = new BizUserLifeCircle();
        ...
        String bizno = CacheConstant.LT_PROVINCE_MAP.get(entUserInfoHistory.getRpcode());
        if(StringUtil.isNotBlank(bizno)){
            bizUserLifeCircle.setBizNo( bizno); // 业务编号
        }else {
            throw new CustomException("PROVICE_BIZ 中rpcode对应的Bizno不存在");
    ...
    private void insertUserInfoHistoryQueue(List<UserInfoHistory> userInfoHistoryList){
        ...
        for (UserInfoHistory userInfoHistory: userInfoHistoryList) {
            ...
            try {
                bizUserLifeCircle = unicomUserHistoryToBoss(userInfoHistory);
            }catch (Exception e){
                log.debug("联通分省个人用户历史字段对应boss字段异常 = {},userInfoHistory = {}",e,userInfoHistory);
                errorInfo = "联通分省个人用户历史对应boss字段异常:"+ e.getMessage() + " userInfoHistory:"+userInfoHistory;
            }
            ...
            bossUserLifeInfo.setBizname("分省联通个人");
    ...
    private void insertEntUserInfoHistoryQueue(List<EntUserInfoHistory> entUserInfoHistories){
        ...
        for (EntUserInfoHistory entUserInfoHistory: entUserInfoHistories) {
            ...
            String errorInfo = null ; // 错误信息
            try {
                bizUserLifeCircle = unicomEntUserHistoryToBoss(entUserInfoHistory);
            }catch (Exception e){
                log.debug("联通分省企业用户历史字段对应boss字段异常 = {},entUserInfoHistory = {}",e,entUserInfoHistory);
                errorInfo = "联通分省企业用户历史字段对应boss字段异常:"+ e.getMessage() + " entUserInfoHistory:"+entUserInfoHistory;
            }
            ...
            bossUserLifeInfo.setBizname("分省联通企业");
            bossUserLifeInfo.setUuid(uuid);
            bossUserLifeInfo.setBizUserLifeCircle(bizUserLifeCircle);
